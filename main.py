...
from discord.commands import SlashCommandGroup  # Импортируем библиотеку групп для глобальных комманд
...
# region •••••••••••••••• ЗЛАЯ ШУТКА
# Данная шутка навеяна игрой в снежки времён IRC
@client.slash_command(guild_ids=guild_ids_for_slash(), default_permission=True, name='joke', description='Злобно пошутить над пользователем.')
async def joke(
    ctx: discord.ApplicationContext,
    user: Option(str, 'Укажжите пользователя через символ "@"')):
    variable = [
        # Гаечный ключ - Попадание
        f'{ctx.author.mention} бросил гаечный ключ в {user} и своим метким попаданием, расколол ему череп! Ракксла находится… - это всё, что успел сказать погибший, залив кровью мастерскую.',

        # Гаечный ключ - Промах
        f'{user} применил технику буддийских монахов и развернул летящий в него гаечный ключ обратно в {ctx.author.mention} проделав в его черепушке приличных размеров вмятину.',

        # Отвёртка - Попадание
        f'{ctx.author.mention} бросил отвёртку в {user}. Бросок был такой силы, что его крошечную голову пробило насквозь.',
        f'{ctx.author.mention} бросил отвёртку в {user} и своим метким попаданием, вскрыл фонтан крови. Срочно несите пакеты с кровью!',

        # Отвёртка - Промах
        f'{ctx.author.mention} попытался бросить отвёртку в {user}, но подскользнулся на машинном масле, тем самым воткнул себе отвёртку в глаз.',

        # Плазменный резак - Промах
        f'{ctx.author.mention} попытался бросить в {user} плазменный резак, но случайно нажал на спусковой крючок и оттяпал себе руку залив себя и всех окружающих кровью.',

        # Молоток - Попадание
        f'{ctx.author.mention} бросил молоток в {user}. Удар был такой силы, что его не спас даже одетый шлем. По крайней мере об этом говорила вытекающая кровь из трещин шлема.',
        
        # Бутылки - Попадание
        f'{ctx.author.mention} бросил пустую бутылку Саксонского вина в {user} и попал ему в голову. Видимо пора вызывать реанимацию, хотя нет, везём в морг.',
        f'{ctx.author.mention} осушил до дна бутылку Серебряного моря Harma и бросил её в {user}. Есть попадание! «Красное море Harma» - новы коктель из осколков и кровавого месива {user}.',
        
        # Бутылки - Промах
        f'{ctx.author.mention} осушил до дна кружку Эля с Kongga и хотел её бросить в {user}, но видимо забыл, что нежелательно пить этот напиток за пределами родного климата из-за чего и обделался в штаны.',

        # Молоток - Промох
        f'{ctx.author.mention} попытался бросить молоток в {user}, но подскользнулся на машинном масле и упал на домкрат. Вот это кровавое месиво, даже не будем смотреть под Sidewinder.',
        f'{ctx.author.mention} бросил молоток в {user}. Бросок был такой силы, что его крошечная голова отказала в стыковке его телу. Кажется, она все еще валяется за ракетной установкой.',]
    await ctx.respond((random.choice(variable)))


# endregion ••••••••••••• ЗЛАЯ ШУТКА // КОНЕЦ
